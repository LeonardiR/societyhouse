...
(function( exports, $ ){
	 ...
	 *
	 * A setting is WordPress data (theme mod, option, header, etc.) that the user can
	...
	api.Control = api.Class.extend({
		 ...
		 * including special handling for nav header items and widgets.
		 *
		 * @since 4.6.0
		...
		getNotificationsContainerElement: function() {
			...
			if ( control.container.hasClass( 'customize-control-nav_menu_item' ) ) {
				control.container.find( '.header-item-settings:first' ).prepend( notificationsContainer );
		...
		addNewPage: function () {
			...
			promise.done( function( data ) {
				...
				// Prepare the new page as an available header item.
				...
				// Add the new item to the list of available header items.
				api.Menus.availableMenuItemsPanel.collection.add( availableItem );
				$content = $( '#available-header-items-post_type-page' ).find( '.available-header-items-list' );
