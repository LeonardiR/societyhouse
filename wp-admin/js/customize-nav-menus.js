...
( function( api, wp, $ ) {
	 ...
	 *
	 * A single available header item model. See PHP's WP_Customize_Nav_Menu_Item_Setting class.
	 *
	 * @constructor
	 ...
	 *
	 * Collection for available header item models.
	 *
	 * @constructor
	 * @augments Backbone.Model
	 */
	 ...
	 *
	 * View class for the available header items panel.
	 *
	 * @constructor
	...
	api.Menus.AvailableMenuItemsPanelView = wp.Backbone.View.extend({
		...
		// Cache current selected header item.
		selected: null,

		// Cache header control that opened the panel.
		...
		initialize: function() {
			...
			this.$search = $( '#header-items-search' );
			this.$clearResults = this.$el.find( '.clear-results' );
			this.sectionContent = this.$el.find( '.available-header-items-list' );

			this.debounceSearch = _.debounce( self.search, 500 );

			_.bindAll( this, 'close' );

			// If the available header items panel is open and the customize controls are
			// interacted with (other than an item being deleted), then close the
			// available header items panel. Also close on back button click.
			$( '#customize-controls, .customize-section-back' ).on( 'click keydown', function( e ) {
				var isDeleteBtn = $( e.target ).is( '.item-delete, .item-delete *' ),
					isAddNewBtn = $( e.target ).is( '.add-new-header-item, .add-new-header-item *' );
			...
			this.$el.on( 'input', '#custom-header-item-name.invalid, #custom-header-item-url.invalid', function() {
			...
			this.sectionContent.scroll( function() {
				var totalHeight = self.$el.find( '.accordion-section.open .available-header-items-list' ).prop( 'scrollHeight' ),
		...
		search: function( event ) {
			var $searchSection = $( '#available-header-items-search' ),
				$otherSections = $( '#available-header-items .accordion-section' ).not( $searchSection );
		...
		doSearch: function( page ) {
			var self = this, params,
				$section = $( '#available-header-items-search' ),
				$content = $section.find( '.accordion-section-content' ),
				itemTemplate = wp.template( 'available-header-item' );
			...
			self.currentRequest = wp.ajax.post( 'search-available-header-items-customizer', params );
		...
		/**
		 * Load available nav header items.
		 *
		 * @since 4.3.0
		...
		loadItems: function( itemTypes, deprecated ) {
			...
			_.each( _itemTypes, function( itemType ) {
				...
				container = $( '#available-header-items-' + itemType.type + '-' + itemType.object );
			...
			request = wp.ajax.post( 'load-available-header-items-customizer', params );
			...
			request.done(function( data ) {
				...
				_.each( data.items, function( typeItems, name ) {
					...
					typeInner = availableMenuItemContainers[ name ].find( '.available-header-items-list' );
		...
		itemSectionHeight: function() {
			...
			sections = this.$el.find( '.accordion-section:not( #available-header-items-search ) .accordion-section-content' );
			lists = this.$el.find( '.accordion-section:not( #available-header-items-search ) .available-header-items-list:not(":only-child")' );
		...
		// Highlights a header item.
		select: function( menuitemTpl ) {
			this.selected = $( menuitemTpl );
			this.selected.siblings( '.header-item-tpl' ).removeClass( 'selected' );
			this.selected.addClass( 'selected' );
		},

		// Highlights a header item on focus.
		focus: function( event ) {
			this.select( $( event.currentTarget ) );
		},

		// Submit handler for keypress and click on header item.
		...
		// Adds a selected header item to the header.
		submit: function( menuitemTpl ) {
			...
			menuitemId = $( this.selected ).data( 'header-item-id' );
			...
			$( menuitemTpl ).find( '.header-item-handle' ).addClass( 'item-added' );
		},

		// Submit handler for keypress and click on custom header item.
		...
		// Adds the custom header item to the header.
		submitLink: function() {
			var menuItem,
				itemName = $( '#custom-header-item-name' ),
				itemUrl = $( '#custom-header-item-url' ),
				urlRegex;
		...
		/**
		 * Creates a new object and adds an associated header item to the header.
		 *
		 * @since 4.7.0
		 * @private
		 *
		...
		submitNew: function( container ) {
			var panel = this,
				...
				title = itemName.val(),
				dataContainer = container.find( '.available-header-items-list' ),
			...
			promise.done( function( data ) {
				...
				availableItem = new api.Menus.AvailableItemModel( {
					'id': 'post-' + data.post_id, // Used for available header item Backbone models.
					'title': itemName.val(),
					'type': itemType,
				...
				$content = container.find( '.available-header-items-list' );
				itemElement = $( wp.template( 'available-header-item' )( availableItem.attributes ) );
				itemElement.find( '.header-item-handle:first' ).addClass( 'item-added' );
		...
		close: function( options ) {
			...
			if ( options.returnFocus && this.currentMenuControl ) {
				this.currentMenuControl.container.find( '.add-new-header-item' ).focus();
			}
			...
			$( '#available-header-items .header-item-handle.item-added' ).removeClass( 'item-added' );
	...
	api.Menus.MenusPanel = api.Panel.extend({
		...
		attachEvents: function() {
			...
			button.on( 'click keydown', function( event ) {
				...
				if ( 'true' === button.attr( 'aria-expanded' ) ) {
					...
					panelMeta.removeClass( 'active-header-screen-options' );
					options.slideUp( 'fast' );
				} else {
					button.attr( 'aria-expanded', 'true' );
					panelMeta.addClass( 'open' );
					panelMeta.addClass( 'active-header-screen-options' );
		...
		ready: function() {
			...
			// Inject additional heading into the header locations section's head container.
			api.section( 'menu_locations', function( section ) {
				section.headContainer.prepend(
					wp.template( 'nav-header-locations-header' )( api.Menus.data )
				);
			} );
		},
	...
	api.Menus.MenuSection = api.Section.extend({
		...
		ready: function() {
			...
			api.bind( 'pane-contents-reflowed', function() {
				...
				section.container.find( '.menu-item .header-item-reorder-nav button' ).attr({ 'tabindex': '0', 'aria-hidden': 'false' });
		...
		populateControls: function() {
			...
			// Add the control for managing the header name.
			...
			// Add the header locations control.
			...
			// Add the control for managing the header auto_add.
			...
			// Add the control for deleting the header
			...
			if ( ! menuDeleteControl ) {
				menuDeleteControl = new api.Control( menuDeleteControlId, {
					section: section.id,
					priority: 1001,
					templateId: 'nav-header-delete-button'
				} );
		...
		updateAssignedLocationsInSectionTitle: function( themeLocationSlugs ) {
			...
			$title.find( '.header-in-location' ).remove();
			_.each( themeLocationSlugs, function( themeLocationSlug ) {
				var $label, locationName;
				$label = $( '<span class="header-in-location"></span>' );
			...
			section.container.toggleClass( 'assigned-to-header-location', 0 !== themeLocationSlugs.length );
		...
		onChangeExpanded: function( expanded, args ) {
			...
			if ( expanded ) {
				...
				$( '#header-to-edit' ).removeAttr( 'id' );
				wpNavMenu.menuList.attr( 'id', 'header-to-edit' ).addClass( 'menu' );
				...
				args.completeCallback = function() {
					if ( 'resolved' !== section.deferred.initSortables.state() ) {
						wpNavMenu.initSortables(); // Depends on header-to-edit ID being set above.
		...
		/**
		 * Highlight how a user may create new header items.
		 *
		 * This method reminds the user to create new header items and how.
		...
		highlightNewItemButton: function() {
			api.utils.highlightButton( this.contentContainer.find( '.add-new-header-item' ), { delay: 2000 } );
		}
	});

	/**
	 * Create a nav header setting and section.
	 *
	 * @since 4.9.0
	 *
	 * @param {string} [name=''] Nav header name.
	 * @returns {wp.customize.Menus.MenuSection} Added nav header.
	 */
	api.Menus.createNavMenu = function createNavMenu( name ) {
		...
		// Register the header control setting.
		...
		/*
		 * Add the header section (and its controls).
	...
	api.Menus.NewMenuSection = api.Section.extend({
		...
		attachEvents: function() {
			...
			section.headContainer.find( '.accordion-section-title' ).replaceWith(
				wp.template( 'nav-header-create-header-section-title' )
			);
			 ...
			 */
			container.on( 'click', '.customize-add-header-button', function() {
				section.expand();
			});

			contentContainer.on( 'keydown', '.header-name-field', function( event ) {
				if ( 13 === event.which ) { // Enter.
					section.submit();
				}
			} );
			contentContainer.on( 'click', '#customize-new-header-submit', function( event ) {
				section.submit();
			...
			function updateNoticeVisibility() {
				container.find( '.add-new-header-notice' ).prop( 'hidden', getNavMenuCount() > 0 );
			}
		...
		populateControls: function() {
			...
			if ( !newMenuSubmitControl ) {
				newMenuSubmitControl = new api.Control( newMenuSubmitControlId, {
					section: section.id,
					priority: 1,
					templateId: 'nav-header-submit-new-button'
				} );
		...
		/**
		 * Create the new header with name and location supplied by the user.
		 *
		 * @since 4.9.0
		 */
		submit: function() {
			var section = this,
				contentContainer = section.contentContainer,
				nameInput = contentContainer.find( '.header-name-field' ).first(),
				name = nameInput.val(),
				menuSection;
			...
			contentContainer.find( '.assigned-header-location input[type=checkbox]' ).each( function() {
				var checkbox = $( this ),
				...
				if ( checkbox.prop( 'checked' ) ) {
					...
					// Reset state for next new header
			...
			// Focus on the new header section.
			menuSection.focus( {
		 ...
		 * creating a header for a specific header location.
		 *
		 * @since 4.9.0
		 *
	 ...
	 *
	 * Customizer control for header locations (rendered as a <select>).
	...
	api.Menus.MenuLocationControl = api.Control.extend({
		...
		ready: function() {
			...
			// Create and Edit header buttons.
			control.container.find( '.create-header' ).on( 'click', function() {
				var addMenuSection = api.section( 'add_menu' );
				addMenuSection.selectDefaultLocation( this.dataset.locationId );
				addMenuSection.focus();
			} );
			control.container.find( '.edit-header' ).on( 'click', function() {
			...
			control.setting.bind( 'change', function() {
				...
				control.container.find( '.create-header' ).toggleClass( 'hidden', menuIsSelected );
				control.container.find( '.edit-header' ).toggleClass( 'hidden', ! menuIsSelected );
			});
	 ...
	 *
	 * Customizer control for header items.
	...
	api.Menus.MenuItemControl = api.Control.extend({
		...
		/**
		 * Show/hide the settings when clicking on the header item handle.
		 */
		_setupControlToggle: function() {
			var control = this;

			this.container.find( '.header-item-handle' ).on( 'click', function( e ) {
				e.preventDefault();
				e.stopPropagation();
		...
		/**
		 * Set up the header-item-reorder-nav
		 */
		_setupReorderUI: function() {
			var control = this, template, $reorderNav;

			template = wp.template( 'header-item-reorder-nav' );

			// Add the header item reordering elements to the header item control.
			control.container.find( '.item-controls' ).after( template );

			// Handle clicks for up/down/left-right on the reorder nav.
			$reorderNav = control.container.find( '.header-item-reorder-nav' );
		...
		/**
		 * Set up event handlers for header item updating.
		 */
		_setupUpdateUI: function() {
			...
			control.elements = {};
			control.elements.url = new api.Element( control.container.find( '.edit-header-item-url' ) );
			control.elements.title = new api.Element( control.container.find( '.edit-header-item-title' ) );
			control.elements.attr_title = new api.Element( control.container.find( '.edit-header-item-attr-title' ) );
			control.elements.target = new api.Element( control.container.find( '.edit-header-item-target' ) );
			control.elements.classes = new api.Element( control.container.find( '.edit-header-item-classes' ) );
			control.elements.xfn = new api.Element( control.container.find( '.edit-header-item-xfn' ) );
			control.elements.description = new api.Element( control.container.find( '.edit-header-item-description' ) );
			...
			control.setting.bind(function( to, from ) {
				...
				if ( false === to ) {
					...
					control.container.find( '.header-item-data-parent-id' ).val( to.menu_item_parent );
		...
		/**
		 * Set up event handlers for header item deletion.
		 */
		_setupRemoveUI: function() {
			...
			$removeBtn.on( 'click', function() {
				// Find an adjacent element to add focus to when this header item goes away
				...
				if ( $next.length ) {
					...
					$adjacentFocusTarget = control.container.nextAll( '.customize-control-nav_menu' ).find( '.add-new-header-item' ).first();
				}
		...
		_setupTitleUI: function() {
			...
			control.container.find( '.edit-header-item-title' ).on( 'blur', function() {
		...
		renderContent: function() {
			...
			containerClasses = [
				'menu-item',
				'header-item-depth-' + String( control.params.depth ),
				'header-item-' + settingValue.object,
				'header-item-edit-inactive'
			];

			if ( settingValue._invalid ) {
				containerClasses.push( 'header-item-invalid' );
		...
		/**
		 * Expand the header item form control.
		 *
		...
		/**
		 * Collapse the header item form control.
		 *
		 * @since 4.5.0 Added params.completeCallback.
		 *
		 * @param {Object}   [params] - Optional params.
		...
		/**
		 * Expand or collapse the header item control.
		 *
		...
		onChangeExpanded: function( showOrHide, params ) {
			...
			$inside = $menuitem.find( '.header-item-settings:first' );
			...
			if ( showOrHide ) {
				// Close all other header item controls before expanding this one.
				...
				complete = function() {
					$menuitem
						.removeClass( 'header-item-edit-inactive' )
						.addClass( 'header-item-edit-active' );
				...
				complete = function() {
					$menuitem
						.addClass( 'header-item-edit-inactive' )
						.removeClass( 'header-item-edit-active' );
		...
		/**
		 * Expand the containing header section, expand the form, and focus on
		...
		focus: function( params ) {
			...
			focusControl = function() {
				...
				params.completeCallback = function() {
					...
					focusable = control.container.find( '.header-item-settings' ).find( 'input, select, textarea, button, object, a[href], [tabindex]' ).filter( ':visible' );
		...
		/**
		 * Move header item up one in the header.
		 */
		moveUp: function() {
		...
		/**
		 * Move header item and all children up one level of depth.
		 */
		moveLeft: function() {
			this._changeDepth( -1 );
		...
		/**
		 * Move header item and children one level deeper, as a submenu of the previous item.
		 */
		moveRight: function() {
			this._changeDepth( 1 );
		...
		_changePosition: function( offset ) {
			...
			if ( ( realPosition === 0 && offset < 0 ) || ( realPosition === siblingSettings.length - 1 && offset > 0 ) ) {
				// @todo Should we allow a header item to be moved up to break it out of a parent? Adopt with previous or following parent?
				return;
			}

			// Update any adjacent header item setting to take on this item's position.
	 ...
	 *
	 * Customizer control for a nav header's name.
	 *
	 * @constructor
	 * @augments wp.customize.Control
	 */
	api.Menus.MenuNameControl = api.Control.extend({
		...
		ready: function() {
			...
			if ( control.setting ) {
				...
				control.nameElement = new api.Element( control.container.find( '.header-name-field' ) );
	 ...
	 *
	 * Customizer control for a nav header's locations.
	 *
	 * @since 4.9.0
	 * @constructor
	 * @augments wp.customize.Control
	...
	api.Menus.MenuLocationsControl = api.Control.extend({
		...
		ready: function () {
			...
			control.container.find( '.assigned-header-location' ).each(function() {
				var container = $( this ),
		 ...
		 * set the default location for a new header.
		 *
		 * @since 4.9.0
		 *
		...
		setSelections: function( selections ) {
			this.container.find( '.header-location' ).each( function( i, checkboxNode ) {
	 ...
	 *
	 * Customizer control for a nav header's auto add.
	 *
	 * @constructor
	 * @augments wp.customize.Control
	 */
	...
	api.Menus.MenuControl = api.Control.extend({
		...
		ready: function() {
			...
			// Add header to Navigation Menu widgets.
			if ( menu ) {
				name = displayNavMenuName( menu.name );

				// Add the header to the existing controls.
				api.control.each( function( widgetControl ) {
					...
					widgetControl.container.find( '.nav-header-widget-form-controls:first' ).show();
					widgetControl.container.find( '.nav-header-widget-no-menus-message:first' ).hide();
				...
				// Add the header to the widget template.
				widgetTemplate = $( '#available-widgets-list .widget-tpl:has( input.id_base[ value=nav_menu ] )' );
				widgetTemplate.find( '.nav-header-widget-form-controls:first' ).show();
				widgetTemplate.find( '.nav-header-widget-no-menus-message:first' ).hide();
			...
			/*
			 * Wait for header items to be added.
		...
		/**
		 * Update ordering of header item controls when the setting is updated.
		 */
		...
		/**
		 * Allow items in each header to be re-ordered, and for the order to be previewed.
		 *
			...
			menuList.on( 'sortstop', function() {
				setTimeout( function() { // Next tick.
					...
					_.each( menuItemControls, function( menuItemControl ) {
						...
						// Note that wpNavMenu will be setting this .header-item-data-parent-id input's value.
						setting.menu_item_parent = parseInt( menuItemControl.container.find( '.header-item-data-parent-id' ).val(), 10 );
		...
		/**
		 * Set up UI for adding a new header item.
		 */
		_setupAddition: function() {
			var self = this;

			this.container.find( '.add-new-header-item' ).on( 'click', function( event ) {
		...
		_handleDeletion: function() {
			...
			// Remove the header from any Navigation Menu widgets.
			api.control.each(function( widgetControl ) {
				if ( ! widgetControl.extended( api.controlConstructor.widget_form ) || 'nav_menu' !== widgetControl.params.widget_id_base ) {
					return;
				}
				...
				widgetControl.container.find( '.nav-header-widget-form-controls:first' ).toggle( 0 !== navMenuCount );
				widgetControl.container.find( '.nav-header-widget-no-menus-message:first' ).toggle( 0 === navMenuCount );
				widgetControl.container.find( 'option[value=' + String( menuId ) + ']' ).remove();
			});

			// Remove the header to the nav header widget template.
			widgetTemplate = $( '#available-widgets-list .widget-tpl:has( input.id_base[ value=nav_menu ] )' );
			widgetTemplate.find( '.nav-header-widget-form-controls:first' ).toggle( 0 !== navMenuCount );
			widgetTemplate.find( '.nav-header-widget-no-menus-message:first' ).toggle( 0 === navMenuCount );
			widgetTemplate.find( 'option[value=' + String( menuId ) + ']' ).remove();
		},

		/**
		 * Update Section Title as header name is changed.
		 */
		_setupTitle: function() {
			...
			control.setting.bind( function( menu ) {
				...
				var section = api.section( control.section() ),
					...
					location = section.headContainer.find( '.header-in-location' ),
				...
				// Update the nav header name in location selects.
				api.control.each( function( control ) {
				...
				// Update the nav header name in all location checkboxes.
				section.contentContainer.find( '.customize-control-checkbox input' ).each( function() {
					if ( $( this ).prop( 'checked' ) ) {
						$( '.current-header-location-name-' + $( this ).data( 'location-id' ) ).text( name );
					}
				} );
			} );
		},
		...
		toggleReordering: function( showOrHide ) {
			var addNewItemBtn = this.container.find( '.add-new-header-item' ),
		...
		/**
		 * Make sure that each header item control has the proper depth.
		 */
		reflowMenuItems: function() {
			...
			reflowRecursively = function( context ) {
				...
				_.each( currentMenuItemControls, function( menuItemControl ) {
					...
					// Update depth.
					if ( ! menuItemControl.container.hasClass( 'header-item-depth-' + String( context.currentDepth ) ) ) {
						...
						menuItemControl.container.addClass( 'header-item-depth-' + String( context.currentDepth ) );
					}
		 ...
		 * changes are made at once, for instance when moving a header item that
		...
		/**
		 * Show an invitation to add new header items when there are no header items.
		 *
		 * @since 4.9.0
		 *
		...
		updateInvitationVisibility: function ( optionalMenuItemControls ) {
			...
			this.container.find( '.new-header-item-invitation' ).toggle( menuItemControls.length === 0 );
		}
	} );
	 ...
	 * @constructor
	 * @augments wp.customize.Control
	 * @deprecated 4.9.0 This class is no longer used due to new header creation UX.
	 */
	api.Menus.NewMenuControl = api.Control.extend({
		...
		_bindHandlers: function() {
			var self = this,
				...
				submit = $( '#create-new-header-submit' );
		...
		/**
		 * Create the new header with the name supplied.
		 *
		 * @deprecated 4.9.0
		 */
		submit: function() {
			...
			var control = this,
				container = control.container.closest( '.accordion-section-new-header' ),
				nameInput = container.find( '.header-name-field' ).first(),
				name = nameInput.val(),
				menuSection;
	...
	/**
	 * Extends wp.customize.sectionConstructor with section constructor for header.
	 */
	...
	api.bind( 'ready', function() {
		...
		// Open and focus header control.
		api.previewer.bind( 'focus-nav-header-item-control', api.Menus.focusMenuItemControl );
	} );
	...
	api.Menus.applySavedData = function( data ) {
		...
		_( data.nav_menu_updates ).each(function( update ) {
			...
			if ( 'inserted' === update.status ) {
				...
				// Add new control for the new header.
				...
				// Update the nav_menu widget to reflect removed placeholder header.
				navMenuCount = 0;
				api.each(function( setting ) {
					if ( /^nav_menu\[/.test( setting.id ) && false !== setting() ) {
						navMenuCount += 1;
		...
		_( data.nav_menu_item_updates ).each(function( update ) {
			...
			if ( 'inserted' === update.status ) {
				...
				// If the parent header item was also inserted, update the menu_item_parent to the new ID.
				...
				// If the header was also inserted, then make sure it uses the new header ID for nav_menu_term_id.
				...
				newControl.container.toggleClass( 'header-item-edit-inactive', oldControl.container.hasClass( 'header-item-edit-inactive' ) );
			}
		});
		...
		_.each( data.widget_nav_menu_updates, function( widgetSettingValue, widgetSettingId ) {
			...
			if ( setting ) {
				...
				setting.preview(); // Send to the preview now so that header refresh will use the inserted header.
			}
		});
	};

	/**
	 * Focus a header item control.
	 *
	...
	/**
	 * Get the control for a given header.
	 *
	 * @param menuId
	...
	/**
	 * Given a header item ID, get the control associated with it.
	 *
	 * @param {string} menuItemId
	 * @return {object|null}
	 */
