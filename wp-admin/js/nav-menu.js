...
(function($) {
	...
	api = wpNavMenu = {
			...
			$.fn.extend({
				...
				updateDepthClass : function(current, prev) {
					return this.each(function(){
						...
						$(this).removeClass('header-item-depth-'+ prev )
							.addClass('header-item-depth-'+ current );
					});
				},
				shiftDepthClass : function(change) {
					return this.each(function(){
						...
						t.removeClass( 'header-item-depth-'+ depth )
							.addClass( 'header-item-depth-'+ ( newDepth ) );
				...
				shiftHorizontally : function( dir ) {
					return this.each(function(){
						...
						// Change .header-item-depth-n class
				...
				updateParentMenuItemDBId : function() {
					return this.each(function(){
						var item = $(this),
							input = item.find( '.header-item-data-parent-id' ),
							depth = parseInt( item.menuItemDepth(), 10 ),
							parentDepth = depth - 1,
							parent = item.prevAll( '.header-item-depth-' + parentDepth ).first();

						if ( 0 === depth ) { // Item is on the top level, has no parent
							input.val(0);
						} else { // Find the parent item, and retrieve its object id.
							input.val( parent.find( '.header-item-data-db-id' ).val() );
						}
					});
				},
				...
				/**
				 * Adds selected header items to the header.
				 *
				 * @param jQuery metabox The metabox jQuery object.
				 */
				addSelectedToMenu : function(processMethod) {
					if ( 0 === $('#header-to-edit').length ) {
						return false;
					}
					...
					return this.each(function() {
						...
						$(checkboxes).each(function(){
							...
							menuItems[listItemDBID] = t.closest('li').getItemData( 'add-header-item', listItemDBID );
						});
				...
				getItemData : function( itemType, id ) {
					...
					fields = [
						'header-item-db-id',
						'header-item-object-id',
						'header-item-object',
						'header-item-parent-id',
					...
					if( !id && itemType == 'menu-item' ) {
						id = this.find('.header-item-data-db-id').val();
					}
					...
					this.find('input').each(function() {
						...
						while ( i-- ) {
							if( itemType == 'menu-item' )
								field = fields[i] + '[' + id + ']';
							else if( itemType == 'add-header-item' )
								field = 'header-item[' + id + '][' + fields[i] + ']';
				...
				setItemData : function( itemData, itemType, id ) { // Can take a type, such as 'header-item', or an id.
					itemType = itemType || 'menu-item';

					if( !id && itemType == 'menu-item' ) {
						id = $('.header-item-data-db-id', this).val();
					}
		...
		countMenuItems : function( depth ) {
			return $( '.header-item-depth-' + depth ).length;
		},

		moveMenuItem : function( $this, dir ) {

			var items, newItemPosition, newDepth,
				menuItems = $( '#header-to-edit li' ),
				...
				prevItemId = prevItem.getItemData()['header-item-db-id'];
			...
			case 'right':
				...
				if ( thisItemData['header-item-parent-id'] === prevItemId )
					break;
				thisItem.shiftHorizontally( 1 );
				break;
			}
		...
		initAccessibility : function() {
			...
			menu.on( 'click', '.menus-move', function () {
				...
				if ( 'undefined' !== typeof dir ) {
					api.moveMenuItem( $( this ).parents( 'li.header-item' ).find( 'a.item-edit' ), dir );
				}
			});
		},
		 ...
		 *
		 * Refreshes advanced accessibility buttons for one header item.
		 * Shows or hides buttons based on the location of the header item.
		 *
		 * @param  {object} itemToRefresh The header item that might need its advanced accessibility buttons refreshed
		 */
		refreshAdvancedAccessibilityOfItem : function( itemToRefresh ) {
			...
			var thisLink, thisLinkText, primaryItems, itemPosition, title,
				...
				itemName = $this.closest( '.header-item-handle' ).find( '.menu-item-title' ).text(),
				position = parseInt( menuItem.index(), 10 ),
				prevItemDepth = ( isPrimaryMenuItem ) ? depth : parseInt( depth - 1, 10 ),
				prevItemNameLeft = menuItem.prevAll('.header-item-depth-' + prevItemDepth).first().find( '.menu-item-title' ).text(),
				prevItemNameRight = menuItem.prevAll('.header-item-depth-' + depth).first().find( '.menu-item-title' ).text(),
				totalMenuItems = $('#header-to-edit li').length,
				hasSameDepthSibling = menuItem.nextAll( '.header-item-depth-' + depth ).length;

				menuItem.find( '.field-move' ).toggle( totalMenuItems > 1 );

			// Where can they move this header item?
			if ( 0 !== position ) {
			...
			if ( 0 !== position ) {
				if ( menuItem.find( '.header-item-data-parent-id' ).val() !== menuItem.prev().find( '.header-item-data-db-id' ).val() ) {
			...
			if ( isPrimaryMenuItem ) {
				primaryItems = $( '.header-item-depth-0' ),
				itemPosition = primaryItems.index( menuItem ) + 1,
				totalMenuItems = primaryItems.length,

				// String together help text for primary header items
				title = menus.menuFocus.replace( '%1$s', itemName ).replace( '%2$d', itemPosition ).replace( '%3$d', totalMenuItems );
			} else {
				parentItem = menuItem.prevAll( '.header-item-depth-' + parseInt( depth - 1, 10 ) ).first(),
				parentItemId = parentItem.find( '.header-item-data-db-id' ).val(),
				parentItemName = parentItem.find( '.menu-item-title' ).text(),
				subItems = $( '.menu-item .header-item-data-parent-id[value="' + parentItemId + '"]' ),
				itemPosition = $( subItems.parents('.menu-item').get().reverse() ).index( menuItem ) + 1;

				// String together help text for sub header items
		...
		refreshAdvancedAccessibility : function() {
			...
			$( '.header-item-settings .field-move .menus-move' ).hide();

			// Mark all header items as unprocessed
			$( 'a.item-edit' ).data( 'needs_accessibility_refresh', true );

			// All open items have to be refreshed or they will show no links
			$( '.header-item-edit-active a.item-edit' ).each( function() {
				api.refreshAdvancedAccessibilityOfItem( this );
			} );
		},
		...
		refreshKeyboardAccessibility : function() {
			$( 'a.item-edit' ).off( 'focus' ).on( 'focus', function(){
				$(this).off( 'keydown' ).on( 'keydown', function(e){
					...
					// Bail if there is only one header item
					if ( 1 === $('#header-to-edit li').length )
						return;
					...
					// Put focus back on same header item
					$( '#edit-' + thisItemData['header-item-db-id'] ).focus();
					return false;
				});
			});
		},
		...
		initPreviewing : function() {
			...
			$( '#header-to-edit' ).on( 'change input', '.edit-header-item-title', function(e) {
		...
		initSortables : function() {
			...
			api.menuList.sortable({
				...
				start: function(e, ui) {
					...
					transport = ui.item.children('.header-item-transport');
					...
					width = ui.helper.find('.header-item-handle').outerWidth(); // Get original width
					width += api.depthToPx(maxChildDepth - originalDepth); // Account for children
					width -= 2; // Subtract 2 for borders
					ui.placeholder.width(width);

					// Update the list of header items.
					tempHolder = ui.placeholder.next( '.menu-item' );
					tempHolder.css( 'margin-top', helperHeight + 'px' ); // Set the margin to absorb the placeholder
					ui.placeholder.detach(); // detach or jQuery UI will think the placeholder is a header item
				...
				change: function(e, ui) {
					// Make sure the placeholder is inside the header.
			...
			function updateMenuMaxDepth( depthChange ) {
				...
				if ( depthChange === 0 ) {
					...
					while( ! $('.header-item-depth-' + newDepth, api.menuList).length && newDepth > 0 )
						newDepth--;
				}
				// Update the depth class.
				body.removeClass( 'header-max-depth-' + menuMaxDepth ).addClass( 'header-max-depth-' + newDepth );
				menuMaxDepth = newDepth;
			}
		},

		initManageLocations : function () {
			$('#header-locations-wrap form').submit(function(){
				window.onbeforeunload = null;
			});
			$('.header-location-menus select').on('change', function () {
				var editLink = $(this).closest('tr').find('.locations-edit-header-link');
		...
		attachMenuEditListeners : function() {
			var that = this;
			$('#update-nav-header').bind('click', function(e) {
				if ( e.target && e.target.className ) {
					if ( -1 != e.target.className.indexOf('item-edit') ) {
						return that.eventOnClickEditLink(e.target);
					} else if ( -1 != e.target.className.indexOf('header-save') ) {
						return that.eventOnClickMenuSave(e.target);
					} else if ( -1 != e.target.className.indexOf('header-delete') ) {
		...
		attachMenuSaveSubmitListeners : function() {
			/*
			 * When a navigation header is saved, store a JSON representation of all form data
			 * in a single input to avoid PHP `max_input_vars` limitations. See #14134.
			 */
			$( '#update-nav-header' ).submit( function() {
				var navMenuData = $( '#update-nav-header' ).serializeArray();
				$( '[name="nav-header-data"]' ).val( JSON.stringify( navMenuData ) );
			});
		},

		attachThemeLocationsListeners : function() {
			var loc = $('#nav-header-theme-locations'), params = {};
			params.action = 'header-locations-save';
			params['menu-settings-column-nonce'] = $('#header-settings-column-nonce').val();
		...
		attachQuickSearchListeners : function() {
			...
			$( '#nav-header-meta' ).on( 'submit', function( event ) {
			...
			$( '#nav-header-meta' ).on( inputEvent, '.quick-search', function() {
				var $this = $( this );
		...
		updateQuickSearchResults : function(input) {
			...
			params = {
				'action': 'header-quick-search',
				'response-format': 'markup',
				'menu': $('#header').val(),
				'menu-settings-column-nonce': $('#header-settings-column-nonce').val(),
				'q': q,
		...
		addCustomLink : function( processMethod ) {
			var url = $('#custom-header-item-url').val(),
				label = $('#custom-header-item-name').val();
			...
			this.addLinkToMenu( url, label, processMethod, function() {
				...
				$('#custom-header-item-name').val('').blur();
				$('#custom-header-item-url').val('http://');
			});
		},
		...
		addItemToMenu : function(menuItem, processMethod, callback) {
			var menu = $('#header').val(),
				nonce = $('#header-settings-column-nonce').val(),
				params;
			...
			$.post( ajaxurl, params, function(menuMarkup) {
				...
				$( '.drag-instructions' ).show();
				if( ! ins.hasClass( 'header-instructions-inactive' ) && ins.siblings().length )
					ins.addClass( 'header-instructions-inactive' );
		...
		/**
		 * Process the add header item request response into header list item. Appends to header.
		 *
		 * @param {string} menuMarkup The text server response of header item markup.
		 *
		 * @fires document#header-item-added Passes menuMarkup as a jQuery object.
		 */
		addMenuItemToBottom : function( menuMarkup ) {
			...
			api.refreshKeyboardAccessibility();
			api.refreshAdvancedAccessibility();
			$( document ).trigger( 'header-item-added', [ $menuMarkup ] );
		},

		/**
		 * Process the add header item request response into header list item. Prepends to header.
		 *
		 * @param {string} menuMarkup The text server response of header item markup.
		 *
		 * @fires document#header-item-added Passes menuMarkup as a jQuery object.
		...
		attachUnsavedChangesListener : function() {
			$('#header-management input, #header-management select, #header-management, #header-management textarea, .header-location-menus select').change(function(){
				api.registerChange();
			});

			if ( 0 !== $('#header-to-edit').length || 0 !== $('.header-location-menus select').length ) {
				...
				$( '#header-settings-column' ).find( 'input,select' ).end().find( 'a' ).attr( 'href', '#' ).unbind( 'click' );
			}
		},
		...
		attachTabsPanelListeners : function() {
			$('#header-settings-column').bind('click', function(e) {
				...
				if ( target.hasClass('nav-tab-link') ) {
					...
					if ( ! wrapper.find( '.tabs-panel-active .menu-item-title' ).length ) {
						wrapper.addClass( 'has-no-header-item' );
					} else {
						wrapper.removeClass( 'has-no-header-item' );
					}
					...
					if ( selectAreaMatch && selectAreaMatch[1] ) {
						items = $('#' + selectAreaMatch[1] + ' .tabs-panel-active .header-item-title input');
				...
				} else if ( target.hasClass('submit-add-to-header') ) {
					api.registerChange();
			 ...
			 */
			$( '#nav-header-meta' ).on( 'click', 'a.page-numbers', function() {
				var $container = $( this ).closest( '.inside' );

				$.post( ajaxurl, this.href.replace( /.*\?/, '' ).replace( /action=([^&]*)/, '' ) + '&action=header-get-metabox',
					function( resp ) {
						...
						// Get the post type header meta box to update.
						...
						// Update the post type header meta box with new content from the response.
		...
		eventOnClickEditLink : function(clickedEl) {
			...
			if ( matchedSection && matchedSection[1] ) {
				...
				if( 0 !== item.length ) {
					if( item.hasClass('header-item-edit-inactive') ) {
						if( ! settings.data('header-item-data') ) {
							settings.data( 'header-item-data', settings.getItemData() );
						}
						settings.slideDown('fast');
						item.removeClass('header-item-edit-inactive')
							.addClass('header-item-edit-active');
					} else {
						settings.slideUp('fast');
						item.removeClass('header-item-edit-active')
		...
		eventOnClickCancelLink : function(clickedEl) {
			var settings = $( clickedEl ).closest( '.header-item-settings' ),
				thisMenuItem = $( clickedEl ).closest( '.header-item' );
			thisMenuItem.removeClass('header-item-edit-active').addClass('header-item-edit-inactive');
			settings.setItemData( settings.data('header-item-data') ).hide();
			return false;
		},

		eventOnClickMenuSave : function() {
			var locs = '',
			menuName = $('#header-name'),
			menuNameVal = menuName.val();
			// Cancel and warn if invalid header name
			if( !menuNameVal || menuNameVal == menuName.attr('title') || !menuNameVal.replace(/\s+/, '') ) {
				menuName.parent().addClass('form-invalid');
				return false;
			}
			// Copy header theme locations
			$('#nav-header-theme-locations select').each(function() {
				locs += '<input type="hidden" name="' + this.name + '" value="' + $(this).val() + '" />';
			});
			$('#update-nav-header').append( locs );
			// Update header item position data
			api.menuList.find('.header-item-data-position').val( function(index) { return index + 1; } );
			window.onbeforeunload = null;
		...
		eventOnClickMenuItemDelete : function(clickedEl) {
			...
			api.removeMenuItem( $('#header-item-' + itemID) );
			api.registerChange();
			return false;
		},
		...
		processQuickSearchQueryResponse : function(resp, req, panel) {
			...
			$items.each(function(){
				...
				if ( matched && matched[1] ) {
					newID = matched[1];
					while( form.elements['header-item[' + newID + '][header-item-type]'] || takenIDs[ newID ] ) {
						newID--;
					}
		 ...
		 *
		 * @fires document#header-removing-item Passes the element to be removed.
		 */
		removeMenuItem : function(el) {
			var children = el.childMenuItems();

			$( document ).trigger( 'header-removing-item', [ el ] );
			el.addClass('deleting').animate({
					...
					if ( 0 === $( '#header-to-edit li' ).length ) {
						...
						ins.removeClass( 'header-instructions-inactive' );
					}
					api.refreshAdvancedAccessibility();
				});
		},
