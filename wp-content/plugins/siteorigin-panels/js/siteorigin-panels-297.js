...
// The utils
panels.utils = {};
panels.utils.menu = require( './utils/header' );
...
module.exports = Backbone.View.extend( {
	wrapperTemplate: _.template( panels.helpers.utils.processTemplate( $( '#siteorigin-panels-context-header' ).html() ) ),
	sectionTemplate: _.template( panels.helpers.utils.processTemplate( $( '#siteorigin-panels-context-header-section' ).html() ) ),
	...
	/**
	 * Listen for the right click context header
	 */
	listenContextMenu: function () {
		...
		$( window ).on( 'contextmenu', function ( e ) {
			...
			if ( thisView.active ) {
				// Lets not double up on the context header
				return true;
			}
	...
	/**
	 * Display the actual context header.
	 *
	 * @param position
	 */
	openMenu: function ( position ) {
		...
		// Start listening for situations when we should close the header
		$( window ).on( 'keyup', {menu: this}, this.keyboardListen );
		$( window ).on( 'click', {menu: this}, this.clickOutsideListen );

		// Set the maximum height of the header
		...
		// position the contextual header
		this.$el.css( {
	...
	closeMenu: function () {
		...
		// Stop listening for situations when we should close the header
	...
	/**
	 * Listen for a click outside the header to close it.
	 * @param e
	 */
	...
	/**
	 * Add a new section to the contextual header.
	 *
	 * @param settings
	 * @param items
	 * @param callback
	...
	addSection: function ( id, settings, items, callback ) {
		...
		var section = $( this.sectionTemplate( {
			settings: settings,
			items: items
		} ) ).attr( 'id', 'panels-header-section-' + id );
	...
	/**
	 * Check if a section exists in the current header.
	 *
	 * @param id
	 * @returns {boolean}
	 */
	hasSection: function( id ){
		return this.$el.find( '#panels-header-section-' + id  ).length > 0;
	},
...
module.exports = Backbone.View.extend( {
	...
	initialize: function ( options ) {
		...
		// Create the context header for this builder
	...
	/**
	 * Activate the contextual header
	 * @param e
	 * @param menu
	 */
	activateContextMenu: function ( e, menu ) {
		...
		if ( $.contains( builder.$el.get( 0 ), e.target ) ) {
			...
			if ( activeView !== undefined && activeView.buildContextualMenu !== undefined ) {
				// We'll pass this to the current active view so it can populate the contextual header
				activeView.buildContextualMenu( e, menu );
			}
			else if ( over.last().hasClass( 'so-panels-welcome-message' ) ) {
				// The user opened the contextual header on the welcome message
	...
	/**
	 * Build the contextual header for the main builder - before any content has been added.
	 */
...
module.exports = Backbone.View.extend( {
	...
	/**
	 * Build up the contextual header for a cell
	 *
	 * @param e
	 * @param menu
	 */
	buildContextualMenu: function ( e, menu ) {
		...
		// Add the contextual header for the parent row
...
module.exports = Backbone.View.extend( {
	...
	/**
	 * Build up the contextual header for a row
	 *
	 * @param e
	 * @param menu
	 */
...
module.exports = Backbone.View.extend( {
	...
	/**
	 * Build up the contextual header for a widget
	 *
	 * @param e
	 * @param menu
	 */
	buildContextualMenu: function ( e, menu ) {
		...
		// Lets also add the contextual header for the entire row
