...
wp.customize.navMenusPreview = wp.customize.MenusCustomizerPreview = ( function( $, _, wp, api ) {
	...
	self.init = function() {
		...
		if ( api.selectiveRefresh ) {
			...
			api.bind( 'add', function( setting ) {
				...
				/*
				 * Handle case where an invalid nav header item (one for which its associated object has been deleted)
				 * is synced from the controls into the preview. Since invalid nav header items are filtered out from
				 ...
				 * this can trigger an infinite fallback refresh when the nav header item lacks any valid items.
				 */
	...
	if ( api.selectiveRefresh ) {
		...
		self.NavMenuInstancePartial = api.selectiveRefresh.Partial.extend(/** @lends wp.customize.navMenusPreview.NavMenuInstancePartial.prototype */{
			 ...
			 * @param {number} [options.params.navMenuArgs.header]
			...
			isRelatedSetting: function( setting, newValue, oldValue ) {
				 ...
				 * nav_menu_item settings is dirty, after a refresh the nav header instance would do a selective
				 ...
				 * @todo Nav header item settings should not include a type_label property to begin with.
				 */
			...
			/**
			 * Make sure that partial fallback behavior is invoked if there is no associated header.
			 *
			 * @since 4.5.0
			 *
			 * @returns {Promise}
			...
			refresh: function() {
				...
				// Make sure the fallback behavior is invoked when the partial is no longer associated with a header.
			...
			renderContent: function( placement ) {
				...
				// Do fallback behavior to refresh preview if header is now empty.
				...
				if ( api.selectiveRefresh.Partial.prototype.renderContent.call( partial, placement ) ) {
					...
					$( document ).trigger( 'customize-preview-header-refreshed', [ {
		...
		/**
		 * Request full refresh if there are nav header instances that lack partials which also match the supplied args.
		 *
		...
		/**
		 * Handle change for nav_menu[] setting for nav header instances lacking partials.
		 *
		 * @since 4.5.0
		 *
		...
		self.onChangeNavMenuSetting = function() {
			...
			// Ensure all nav header instances with a theme_location assigned to this header are handled.
		...
		/**
		 * Handle change for nav_menu_item[] setting for nav header instances lacking partials.
		 *
		 * @since 4.5.0
		 *
		...
		/**
		 * Handle change for nav_menu_locations[] setting for nav header instances lacking partials.
		 *
		 * @since 4.5.0
		 *
		 * @this {wp.customize.Value}
	...
	/**
	 * Connect nav header items with their corresponding controls in the pane.
	 *
	 * Setup shift-click on nav header items which are more granular than the nav header partial itself.
	 * Also this applies even if a nav header is not partial-refreshable.
	 *
	 * @since 4.5.0
	 */
	self.highlightControls = function() {
		...
		// Focus on the header item control when shift+clicking the header item.
		$( document ).on( 'click', selector, function( e ) {
			...
			if ( navMenuItemParts ) {
				e.preventDefault();
				e.stopPropagation(); // Make sure a sub-nav header item will get focused instead of parent items.
				api.preview.send( 'focus-nav-header-item-control', parseInt( navMenuItemParts[1], 10 ) );
			}
		});
	};
