...
wp.customize.selectiveRefresh = ( function( $, api ) {
			...
			function() {
				...
				_.each( self._pendingPartialRequests, function( pending, partialId ) {
					...
					if ( ! self.partial.has( partialId ) ) {
						 ...
						 * the contained sidebar for rendering widgets or what the nav header args are for a header.
						 */
	...
	/**
	 * Add partials for any nav header container elements in the document.
	 *
	...
	self.addPartials = function( rootElement, options ) {
		...
		containerElements.each( function() {
			 ...
			 * handled yet. An example where this would apply is a nav header
			 * embedded inside of a navigation header widget. When the widget's title
			 * is updated, the entire widget will re-render and then the event
			 * will be triggered for the nested nav header to do any initialization.
			 */
