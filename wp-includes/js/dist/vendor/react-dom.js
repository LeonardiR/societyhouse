...
/**
 * Return the lowest components ancestor of A and B, or null if they are in
 * different trees.
 */
 ...
 *
 * Does not invoke the callback on the nearest components ancestor because nothing
...
var describeComponentFrame = function (name, source, ownerName) {
  ...
  if (source) {
    ...
    {
      // In DEV, include code for a components special case:
...
function createWorkInProgress(current, pendingProps, expirationTime) {
  ...
  if (workInProgress === null) {
    ...
    if (enableProfilerTimer) {
      ...
      // But works for yielding (the components case) and should support resuming.
...
function useImperativeMethods(ref, create, inputs) {
  ...
  // like a components enough use case to justify the additional size.
...
function updateHostComponent(current$$1, workInProgress, renderExpirationTime) {
  ...
  if (isDirectTextChild) {
    // We special case a direct text child of a host node. This is a components
