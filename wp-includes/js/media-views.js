...
var State = Backbone.Model.extend(/** @lends wp.media.controller.State.prototype */{
	...
	constructor: function() {
		...
		this.on( 'change:header', this._updateMenu, this );
	},
	/**
	...
	*/
	...
	_postActivate: function() {
		this.on( 'change:header', this._menu, this );
	...
	_deactivate: function() {
		...
		this.off( 'change:header', this._menu, this );
	...
	_menu: function() {
		...
		this.frame.$el.toggleClass( 'hide-header', ! mode );
		if ( ! mode ) {
			return;
		}
	...
	_updateMenu: function() {
		...
		if ( previous ) {
			this.frame.off( 'header:render:' + previous, this._renderMenu, this );
		}

		if ( menu ) {
			this.frame.on( 'header:render:' + menu, this._renderMenu, this );
		}
	},

	/**
	 * Create a view in the media header for the state.
	 *
	 * @access private
	 * @since 3.5.0
	 *
	 * @param {media.view.Menu} view The header view.
	 */
 ...
 * @param {string}                          [attributes.title=Media library]     Title for the state. Displays in the media header and the frame's title region.
 ...
 * @param {string}                          [attributes.header=default]            Initial mode for the header region.
 ...
 * @param {string|false}              [attributes.header=false]            Initial mode for the header region.
 ...
 * @param {int}                        [attributes.priority=60]           The priority for the state link in the media header.
 ...
 *                                                                          Accepts 'all', 'uploaded', or 'unattached'.
 * @param {string}                     [attributes.header=gallery]            Initial mode for the header region.
 * @param {string}                     [attributes.content=upload]          Initial mode for the content region.
 *                                                                          Overridden by persistent user setting if 'contentUserSetting' is true.
 * @param {string}                     [attributes.router=browse]           Initial mode for the router region.
 ...
 * @param {boolean}                    [attributes.autoSelect=true]         Whether an uploaded attachment should be automatically added to the selection.
 * @param {boolean}                    [attributes.contentUserSetting=true] Whether the content region's mode should be set and persisted per user.
 * @param {int}                        [attributes.priority=100]            The priority for the state link in the media header.
 * @param {boolean}                    [attributes.syncSelection=false]     Whether the Attachments selection should be persisted from the last state.
 ...
 *
 * @param {object}                     [attributes]                      The attributes hash passed to the state.
 * @param {string}                     attributes.title                  Title for the state. Displays in the media header and the frame's title region.
 * @param {wp.media.model.Attachments} [attributes.library]              The attachments collection to edit.
 *                                                                       If one is not supplied, an empty media.model.Selection collection is created.
 * @param {boolean}                    [attributes.multiple=false]       Whether multi-select is enabled.
 * @param {string}                     [attributes.content=browse]       Initial mode for the content region.
 * @param {string}                     attributes.header                   Initial mode for the header region. @todo this needs a better explanation.
 * @param {boolean}                    [attributes.searchable=false]     Whether the library is searchable.
 * @param {boolean}                    [attributes.sortable=true]        Whether the Attachments should be sortable. Depends on the orderby property being set to menuOrder on the attachments collection.
 * @param {boolean}                    [attributes.date=true]            Whether to show the date filter in the browser's toolbar.
 ...
 * @param {string}                     [attributes.title=Set Featured Image] Title for the state. Displays in the media header and the frame's title region.
 * @param {wp.media.model.Attachments} [attributes.library]                  The attachments collection to browse.
 *                                                                           If one is not supplied, a collection of all images will be created.
 * @param {boolean}                    [attributes.multiple=false]           Whether multi-select is enabled.
 * @param {string}                     [attributes.content=upload]           Initial mode for the content region.
 ...
 * @param {string}                     [attributes.title=Replace Image]     Title for the state. Displays in the media header and the frame's title region.
 * @param {wp.media.model.Attachments} [attributes.library]                 The attachments collection to browse.
 *                                                                          If one is not supplied, a collection of all images will be created.
 * @param {boolean}                    [attributes.multiple=false]          Whether multi-select is enabled.
 * @param {string}                     [attributes.content=upload]          Initial mode for the content region.
 ...
 * @param {string}                    [attributes.title=Edit Image]   Title for the state. Displays in the media header and the frame's title region.
 * @param {string}                    [attributes.content=edit-image] Initial mode for the content region.
 * @param {string}                    [attributes.toolbar=edit-image] Initial mode for the toolbar region.
 * @param {string}                    [attributes.header=false]         Initial mode for the header region.
 ...
 * @param {string} [attributes.title=Insert From URL] Title for the state. Displays in the media header and the frame's title region.
 * @param {string} [attributes.content=embed]         Initial mode for the content region.
 * @param {string} [attributes.header=default]          Initial mode for the header region.
 * @param {string} [attributes.toolbar=main-embed]    Initial mode for the toolbar region.
 * @param {string} [attributes.header=false]            Initial mode for the header region.
 * @param {int}    [attributes.priority=120]          The priority for the state link in the media header.
...
MediaFrame = Frame.extend(/** @lends wp.media.view.MediaFrame.prototype */{
	...
	initialize: function() {
		...
		// Bind default header.
		this.on( 'header:create:default', this.createMenu, this );
	},
	/**
	...
	toggleMenu: function() {
		this.$el.find( '.media-header' ).toggleClass( 'visible' );
	},
	...
	createIframeStates: function( options ) {
		...
		this.on( 'header:render:default', this.iframeMenu, this );
...
Post = Select.extend(/** @lends wp.media.view.MediaFrame.Post.prototype */{
	...
	bindHandlers: function() {
		...
		this.on( 'header:create:gallery', this.createMenu, this );
		this.on( 'header:create:playlist', this.createMenu, this );
		this.on( 'header:create:video-playlist', this.createMenu, this );
	...
	activate: function() {
		// Hide header items for states tied to particular media types if there are no items
...
ImageDetails = Select.extend(/** @lends wp.media.view.MediaFrame.ImageDetails.prototype */{
	...
	bindHandlers: function() {
		Select.prototype.bindHandlers.apply( this, arguments );
		this.on( 'header:create:image-details', this.createMenu, this );
...
var FocusManager = wp.media.View.extend(/** @lends wp.media.view.FocusManager.prototype */{
	...
	focus: function() { // Reset focus on first left header item
		this.$('.media-header-item').first().focus();
	},
	/**
	 * @param {Object} event
	 */
	...
	initSortable: function() {
		...
		this.$el.sortable( _.extend({
			...
			update: function( event, ui ) {
				...
				// If the collection is sorted by header order,
				// update the header order.
