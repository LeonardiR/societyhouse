	...
	editor.addCommand( 'WP_Help', function() {
		...
		html = html +
			...
			'<table class="wp-help-single">' +
				...
				tr({ 'Alt + F9':  'Editor header (when enabled)' }) +
	...
	editor.on( 'preinit', function() {
		...
		function create( buttons, bottom ) {
			...
			function reposition() {
				...
				// Add offset in iOS to move the header over the image, out of the way of the default iOS header.
				...
				// No up/down arrows on the header over images in iOS.
				if ( tinymce.Env.iOS && currentSelection.nodeName === 'IMG' ) {
