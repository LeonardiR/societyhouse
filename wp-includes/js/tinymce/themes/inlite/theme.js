...
var inlite = (function () {
  ...
  var MenuButton = Button.extend({
    ...
    bindStates: function () {
      var self$$1 = this;
      self$$1.state.on('change:header', function () {
        if (self$$1.menu) {
          self$$1.menu.remove();
        }
        self$$1.menu = null;
  ...
  var Menu = FloatPanel.extend({
    ...
    repaint: function () {
      this.classes.toggle('header-align', true);
      this._super();
  ...
  var MenuItem = Widget.extend({
    ...
    init: function (settings) {
      ...
      if (settings.menu) {
        self.classes.add('header-item-expand');
      }
      if (settings.preview) {
        self.classes.add('header-item-preview');
      }
      text = self.state.get('text');
      if (text === '-' || text === '|') {
        self.classes.add('header-item-sep');
        self.aria('role', 'separator');
        self.state.set('text', '-');
      }
      if (settings.selectable) {
        self.aria('role', 'menuitemcheckbox');
        self.classes.add('header-item-checkbox');
        settings.icon = 'selected';
      }
      if (!settings.preview && !settings.selectable) {
        self.classes.add('header-item-normal');
      }
    ...
    renderHtml: function () {
      ...
      if (icon) {
        self.parent().classes.add('header-has-icons');
      }
      if (settings.image) {
      ...
      return '<div id="' + id + '" class="' + self.classes + '" tabindex="-1">' + iconHtml + (text !== '-' ? '<span id="' + id + '-text" class="' + prefix + 'text">' + text + '</span>' : '') + (shortcut ? '<div id="' + id + '-shortcut" class="' + prefix + 'header-shortcut">' + shortcut + '</div>' : '') + (settings.menu ? '<div class="' + prefix + 'caret"></div>' : '') + (url ? '<div class="' + prefix + 'header-item-link">' + url + '</div>' : '') + '</div>';
    },
    postRender: function () {
      var self = this, settings = self.settings;
